 ...
 */
export const handlePriceUpdateInterval = async (
  ws: WebSocket,
  request: http.IncomingMessage,
) => {
  try {
    const clientData =
      WebSocketMessageClientData.createFromIncomingMessage(request);
    const userWatchlist = await watchlistService.getWatchlist(
